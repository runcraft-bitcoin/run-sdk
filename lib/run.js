...
class Run {
  ...
  constructor(options = {}) {
    ...
    options.trust = parseMockTrust(network, options.trust, keys.includes('trust'))
    ...
    // on mockchain, load the cache with defaults
    if (network === "mock") {
      for (const [key, value] of Object.entries(extra.main.states)) {
        this._kernel._cache.set(key, value)
      }
    }
...
function parseApi(api, specified, blockchain, network) {
  // case: valid apis, we just return as-is
  ...
  if (typeof api === 'undefined' && !specified) {
    ...
    if (typeof blockchain === 'undefined') {
      // todo we could reactivate this by checking for an env var that is a host to a run-like api
...
function parseState(state, specified, network, api, stateServerHost, apiKey) {
  ...
  if (typeof state === 'undefined' && !specified) {
    ...
    const lastStateServerHost = lastState && lastState.stateServerHost
    const lastApiKey = lastState && lastState.apiKey

    // If our last run instance had a state on the same config, reuse it
    if (lastState
      && lastNetwork === network
      && lastApi === api
      && lastStateServerHost === stateServerHost
      && lastApiKey === apiKey
    ) {
      return lastState
    }
    ...
    // If we are on mainnet or testnet, then use a distant State Server
...
// motivation for this attempt was to try to make network:"mock" work on his own without having to pass a trust:'state' or trust:'*'
/** 
  implicitly trust state when network="mock"

  usage: options.trust = parseMockTrust(...)
*/
function parseMockTrust(network, trust, specifiedTrust) {
  const defTrust = Run.defaults.trust

  if (!network || typeof network === 'undefined' || network !== 'mock') return trust  // change nothing

  // from here, network==='mock'
  if (!specifiedTrust) {
    return defTrust || 'state'  // return default if possible, otherwise 'state
  }

  // from here trust is defined
  if (trust === "cache") return 'state'  // 'cache' is deprecated for 'state'

  // for some reason '*' doesn't work without adding 'trust' when network='mock'
  if (trust === "*") return ['*', 'state']
  if (Array.isArray(trust)) {
    if (trust.includes('*') && !trust.includes('state')) {
      trust.push('state')
      return trust
    }
  }

  // the tests run on mock and break if we raise like this, so commenting this block for now
  // one solution could to set Run.defaults.trust = [] to = 'state' by default ?
  // let networkMockMismatch
  // if (Array.isArray(trust)) {
  //   networkMockMismatch = !trust.includes('state') && !trust.includes('cache') && !trust.includes('*')
  // } else {
  //   networkMockMismatch = trust !== 'state' && trust !== 'cache' && trust !== '*'
  // }
  // if (networkMockMismatch) {
  //   throw new Error(`Mock network can only be used if you trust 'state' or '*', got '${trust}' (specified=${specifiedTrust})
  //       Run.defaults.trust=${JSON.stringify(Run.defaults.trust)}`)
  // }
